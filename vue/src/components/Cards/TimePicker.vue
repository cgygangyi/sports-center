...
<!--</style>-->
<!-- Authors Table Card -->
<!--&lt;!&ndash;管理员版本，点击在灰色和红色切换，最后在session里存数组&ndash;&gt;-->
<template>
    <div>
        <table class="calendar-table">
            <thead>
...
<!--            :style="{ backgroundColor: data[index*10+j].open === 0 ? 'gray' : data[index*10+j].free === 0 ? 'red' : 'white' }"-->
            <!--                    :class="{ 'red-cell': data[index*10+j].free === 0 && open[index*10+j] === 1, 'gray-cell': open[index*10+j] === 0}"-->
        </table>
<!--        <table class="calendar-table">-->
<!--            <thead>-->
<!--            <tr>-->
...
<!--            </tr>-->
<!--            </thead>-->
<!--            <tr v-for="(index, i) in timeList" :key="i">-->
<!--            <th :colspan="4">{{ index+9+':00 - ' + (index+10)+':00' }}</th>-->
<!--            <th-->
<!--                :colspan="4"-->
<!--                :id="index"-->
...
export default {
    ...
    //         let dataString = this.data[t].begin
    //         if (dataString[5] === '0') {
    //             dataString = dataString[0] + dataString[1] + dataString[2] + dataString[3] + dataString[4] + dataString[6] + dataString[7] + dataString[8] + dataString[9]
    //         } else {
    //             dataString = dataString[0] + dataString[1] + dataString[2] + dataString[3] + dataString[4] + dataString[5] + dataString[6] + dataString[7] + dataString[8] + dataString[9]
    //         }
    //
    //         dataString = dataString.replace('-', '.')
    //         dataString = dataString.replace('-', '.')
    ...
    methods: {
        ...
        //     if (!(this.data[id].open === 1 && this.data[id].free === 0)) {
        //         console.log(id)
        //         if (this.open[id] === 0) { // 如果该单元格未被预定，切换颜色并更新 booked 属性
        //             this.open[id] = 1
        //             const change = document.getElementById(id)
        //             // //
        //             // // change.innerHTML = '已预定'
